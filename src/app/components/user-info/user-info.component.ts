...
export class UserInfoComponent implements OnInit {
  ...
  getUserInfo(username: string): void {
    this.requestService.getUserInfo(username).subscribe(
      ...
      // if an error occurred, redirect admin to previous page
      error => {
        this.redirectAdmin()
      }
    )
  ...
  // approve the said user's registration request and redirect admin to previous page
  approveUser(): void {
  ...
  redirectAdmin(): void {
    let returnUrl = this.route.snapshot.queryParamMap.get('returnUrl') || 'panel/administration/users';
